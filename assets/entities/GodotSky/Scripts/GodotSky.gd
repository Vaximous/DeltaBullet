...
@onready var sunModel = $SunMoon/Sun
@onready var moonModel = $SunMoon/Moon
@export_range(0,2400,0.01) var timeOfDay : float = 1200.0:
	set(value):
		timeOfDay = value
		checkPlanetPosition()
@export var simulateTime : bool = false
@export_range(0.000000,10,0.000001) var rateOfTime : float = 0.1
...
@export var skyPreset : SkyPreset = preload("res://assets/entities/GodotSky/Presets/sky_default.tres")
...
func updateGraphics():
	get_environment().ssil_enabled = UserConfig.graphics_Ssil
	get_environment().ssr_enabled = UserConfig.graphics_Ssr
	get_environment().sdfgi_enabled = UserConfig.graphics_Sdfgi

func checkPlanetPosition():
